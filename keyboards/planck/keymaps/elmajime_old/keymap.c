...
enum planck_layers { _BEPO, _QWERTY, _COLEMAK, _LOWER, _RAISE, _PLOVER, _ADJUST };

enum planck_keycodes { BEPO = SAFE_RANGE, QWERTY, COLEMAK, PLOVER, BACKLIT, EXT_PLV, EXCL_CHAP, COMMA_SEMI, QUEST_TICK, DOT_DOUBLE };
...
void TD_CURRENCIES_finished(tap_dance_state_t *state, void *user_data);
void TD_CURRENCIES_reset(tap_dance_state_t *state, void *user_data);

void TD_LALT_RALT_finished(tap_dance_state_t *state, void *user_data);
void TD_LALT_RALT_reset(tap_dance_state_t *state, void *user_data);

void TD_RPAREN_RAISE_finished(tap_dance_state_t *state, void *user_data);
void TD_RPAREN_RAISE_reset(tap_dance_state_t *state, void *user_data);

void TD_CHAPEAU_finished(tap_dance_state_t *state, void *user_data);
void TD_CHAPEAU_reset(tap_dance_state_t *state, void *user_data);

void TD_LPAREN_LOWER_finished(tap_dance_state_t *state, void *user_data);
void TD_LPAREN_LOWER_reset(tap_dance_state_t *state, void *user_data);
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    /* Qwerty
     * ,-----------------------------------------------------------------------------------.
     * |  Esc |   Q  |   W  |   E  |   R  |   T  |   Y  |   U  |   I  |   O  |   P  | Bpc  |
     * |------+------+------+------+------+------+------+------+------+------+------+------|
     * |  Tab |   A  |   S  |   D  |   F  |   G  |   H  |   J  |   K  |   L  |   ;  | Sup  |
     * |------+------+------+------+------+------+------+------+------+------+------+------|
     * | Shft |   Z  |   X  |   C  |   V  |   B  |   N  |   M  |   ,  |   .  |   /  | Ent  |
     * |------+------+------+------+------+------+------+------+------+------+------+------|
     * | Ctrl |  Fn  | Win  |  Alt | Space|      |      | Ent  | Agr  |      |      | Shft |
     * `-----------------------------------------------------------------------------------'
     */
     ...
     * `-----------------------------------------------------------------------------------'
     */
    [_ADJUST] = LAYOUT_planck_grid(_______, QK_BOOT, DB_TOGG, RGB_TOG, RGB_MOD, RGB_HUI, RGB_HUD, RGB_SAI, RGB_SAD, RGB_VAI, RGB_VAD, KC_DEL, _______, BACKLIT, _______, AU_ON, AU_OFF, AG_NORM, AG_SWAP, BEPO, QWERTY, COLEMAK, PLOVER, _______, _______, _______, _______, _______, _______, MI_ON, MI_OFF, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______)
